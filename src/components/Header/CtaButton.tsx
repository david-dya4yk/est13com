...
export default function CtaButton({
  ...
  const chars = Array.from(label).map((c) => (c === " " ? " " : c));

  return (
    <Link href={href} className={s.button} aria-label={label}>
      <div className={s.bg} />
      ...
      <div className={s.wrap}>
        ...
        <div className={s.content}>
          <span className={`${s.char} ${s.state1}`} aria-hidden>
            {chars.map((c, i) => (
              <span key={i} data-label={c} style={{ "--i": i + 1 } as CSSProperties}>
                {c}
              </span>
            ))}
          </span>
